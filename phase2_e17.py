"""Phase 2: E17 (Livermore pivot-structure breakout) registered
evaluation. Single registered run per HYPOTHESES.md -- do not re-run
after seeing output.

Primary/default cell: pivot_left=pivot_right=10 (indicator's own shipped
default). Plateau cells (7/10/15, left=right jointly) get a lighter
net-positive-only check, matching the E1 precedent.
"""
...
if __name__ == "__main__":
    ...
    print("\n== PLATEAU (net total return by pivot window, long/flat) ==")
    plateau_results = {}
    for pw in (7, 10, 15):
        sig = compute_signals(btc, pivot_left=pw, pivot_right=pw)
        trades, daily, eq = run_backtest(btc, sig, allow_short=False)
        total = trades["ret"].sum() if len(trades) else 0.0
        plateau_results[pw] = total
        print(f"pivot_window={pw:2d}: n={len(trades):3d} total_ret={total:+.4f} "
              f"final={eq.iloc[-1]:.3f}x")

    print("\n== PRIMARY (pivot_window=10, full gate battery, long/flat) ==")
    sig10 = compute_signals(btc, pivot_left=10, pivot_right=10)
    trades, daily, eq = run_backtest(btc, sig10, allow_short=False)
    n = len(trades)
    ...
    print(f"corr(E17, E4-v2) = {corr_e4v2:.4f}  (n={n_e4v2} overlapping days)")
    print(f"corr(E17, E6)    = {corr_e6:.4f}  (n={n_e6} overlapping days)")
    ...
    print(f"correlation (reported/interpreted, not a hard pass/fail here): "
          ...
          f"{'>0.8 rough guide: reads as redundant expression' if max(corr_e4v2, corr_e6) > 0.8 else 'below the 0.8 rough guide, but still substantial'}")
