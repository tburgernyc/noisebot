"""E19 gate evaluation: delta-neutral perp funding-rate harvest, BTC/
ETH/SOL spot+perp hedged (registered 2026-07-25). Single registered run
per HYPOTHESES.md -- do not re-run after seeing output.

Windowing: run_e19 operates over each asset's full price history (pre-
2020 bars come back flat -- no funding data, `held` never turns on).
Gates/Sharpe/bootstrap/correlation are computed only from METRIC_START
(2020-01-01, when funding data begins) onward, matching phase2_e7.py's
convention exactly -- including pre-funding flat padding would dilute
Sharpe and understate both bootstrap ruin risk and the correlation gate.

Primary/default cell: entry=8%/exit=3% (the spec's stated base case).
Plateau cells (6/2, 8/3, 12/5) get a net-positive-only check.
"""
...
from e19_funding_basis import run_e19
...
if __name__ == "__main__":
    ...
    px = load_universe(paths=PATHS)
    print(f"price universe: {px.index[0].date()} -> {px.index[-1].date()} "
          f"({len(px)} rows, cols={list(px.columns)})")

    print("\n== PLATEAU (net total return by entry/exit threshold pair) ==")
    plateau_results = {}
    for entry, exit_thr in PLATEAU:
        trades, daily, eq, per_asset, att = run_e19(px, fund, entry=entry, exit_thr=exit_thr)
    ...
    trades, daily, eq, per_asset, att = run_e19(px, fund, entry=PRIMARY[0], exit_thr=PRIMARY[1])
    n = len(trades)
    pf, wr = pf_of(trades)
    ...
    for nm in SYMS:
        ...
        print(f"  {nm}: n={len(t_a)}  final={e_a.iloc[-1]:.3f}x  "
              f"funding={att_a['funding']:+.4f}  price={att_a['price']:+.2e}  "
    ...
    print(f"corr(E19, E4-v2) = {corr_e4v2:.4f}  (n={n_e4v2} overlapping days, metric window)")
    print(f"corr(E19, E6)    = {corr_e6:.4f}  (n={n_e6} overlapping days, metric window)")
    ...
    print(f"attribution |price|<20%|funding|: price={att['price']:+.2e} "
          f"funding={att['funding']:+.4f} -> {'PASS' if attr_gate else 'FAIL'}")
    ...
    print(f"\nE19 VERDICT: {verdict} ({n_pass}/{len(all_gates)} gates)")
